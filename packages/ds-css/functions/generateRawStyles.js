...
async function processFile(
    ...
    let styleContentResponsive = stylesContent;
    if (responsive && !exclude.includes(file)) {
        styleContentResponsive = await generateResponsiveVariants(stylesContent)
    }

    stylesContent = cleanCss(stylesContent)
    styleContentResponsive = cleanCss(styleContentResponsive)

    if (layer) {
        stylesContent = `@layer ${layer} {\n${stylesContent}\n}`
        styleContentResponsive = `@layer ${layer} {\n${styleContentResponsive}\n}`
    }
    ...
    await fs.writeFile(
        path.join(import.meta.dirname, distDir, `${distDir}/${file}.light.css`),
        stylesContent,
    )
}
